...
def plot_tradeoff_curve(input_csv: Path, output_dir: Path) -> None:
    ...
    Column names match simulation_results.csv output: 'Carbon Reduction', 'SLO Violation Rate (%)'
    """
    logger.info(f"Generating tradeoff curve from {input_csv}...")
    df = pd.read_csv(input_csv)

    # Filter out baseline row (Carbon Reduction == 0.0)
    df_plot = df[df['Carbon Reduction'] > 0].copy()
    ...
    for i, row in df_plot.iterrows():
        plt.text(
            x=row['Carbon Reduction'] + 0.5,
            y=row['SLO Violation Rate (%)'] + 0.5,
            s=row['Policy'],
...
def plot_workload_slo_violations(input_csv: Path, output_dir: Path) -> None:
    ...
    Column name matches per_workload_results.csv: 'SLO_Violation_Rate_%'
    """
...
def main():
    ...
    # Paths — carbon traces are written to outputs/data by simulation.py

...
def adaptive_hybrid(lats, cis, alpha, **kwargs):
    """
    Policy 5: Adaptive Hybrid — uses a dynamically adjusted alpha.
    The alpha value is maintained by a per-workload sliding window
    controller in simulation.py and passed in here per request.
    Same scoring logic as hybrid_policy but alpha is never static.
    alpha=1.0 → pure latency-first
    alpha=0.0 → pure carbon-first
    """
    norm_l = (lats - LATENCY_GLOBAL_MIN) / (LATENCY_GLOBAL_MAX - LATENCY_GLOBAL_MIN)
